...
/**
 * Convert plain cover letter text to PDF via render-cv service.
 * @returns {Promise<{ url: string }>}
 */
export async function generateCoverLetterPdf({ coverLetterText }) {
  const text = String(coverLetterText || '').trim();
  if (!text) throw new Error('coverLetterText is required');

  const tailoredBase = String(config.tailoredCvServiceUrl || '').trim().replace(/\/$/, '');
  if (!tailoredBase) throw new Error('Tailored CV service URL is not configured');

  const upstreamRes = await fetch(`${tailoredBase}/cover-letter-pdf`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  const url = upstreamJson?.url != null ? String(upstreamJson.url).trim() : '';
  if (!url) throw new Error('Cover letter PDF service returned no url');
  return { url };
}
